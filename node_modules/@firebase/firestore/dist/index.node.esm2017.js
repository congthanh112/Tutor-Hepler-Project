...
var version = "7.17.2-20207421553";
...
function fromMutation(serializer, proto) {
    ...
    if (proto.update) {
        ...
        const fieldTransforms = proto.transform.fieldTransforms.map(transform => fromFieldTransform(serializer, transform));
...
IndexedDbRemoteDocumentCache.RemoteDocumentChangeBuffer = class extends RemoteDocumentChangeBuffer {
    ...
    applyChanges(transaction) {
        ...
        this.changes.forEach((key, maybeDocument) => {
            ...
            if (maybeDocument) {
                const doc = toDbRemoteDocument(this.documentCache.serializer, maybeDocument, this.readTime);
                ...
                if (this.trackRemovals) {
                    ...
                    const deletedDoc = toDbRemoteDocument(this.documentCache.serializer, new NoDocument(key, SnapshotVersion.min()), this.readTime);
...
class SimpleDb {
    ...
    static openOrCreate(name, version, schemaConverter) {
        logDebug(LOG_TAG$2, 'Opening database:', name);
        return new PersistencePromise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                logDebug(LOG_TAG$2, 'Database "' + name + '" requires upgrade from version:', event.oldVersion);
                const db = event.target.result;
                schemaConverter
                    .createOrUpgrade(db, request.transaction, event.oldVersion, SCHEMA_VERSION)
                    .next(() => {
                    logDebug(LOG_TAG$2, 'Database upgrade to version ' + SCHEMA_VERSION + ' complete');
                });
            };
        }).toPromise();
    }
    ...
    async runTransaction(mode, objectStores, transactionFn) {
        ...
        while (true) {
            ...
            try {
                ...
                logDebug(LOG_TAG$2, 'Transaction failed with error: %s. Retrying: %s.', error.message, retryable);
                if (!retryable) {
...
class SimpleDbStore {
    ...
    get(key) {
        ...
        return wrapRequest(request).next(result => {
            ...
            logDebug(LOG_TAG$2, 'GET', this.store.name, key, result);
            return result;
        });
    }
    delete(key) {
 ...
 * limitations under the License.
 */
const LOG_TAG$3 = 'LruGarbageCollector';
        ...
        this.gcTask = this.asyncQueue.enqueueAfterDelay("lru_garbage_collection" /* LruGarbageCollection */, delay, async () => {
            ...
            try {
                ...
                if (isIndexedDbTransactionError(e)) {
                    logDebug(LOG_TAG$3, 'Ignoring IndexedDB error during garbage collection: ', e);
                }
                else {
 ...
 * limitations under the License.
 */
const LOG_TAG$4 = 'IndexedDbPersistence';
/**
...
class IndexedDbPersistence {
    ...
    async shutdown() {
        ...
        await this.runTransaction('shutdown', 'readwrite', txn => {
            ...
            logDebug(LOG_TAG$4, 'Proceeding with shutdown despite failure: ', e);
        });
        this.simpleDb.close();
    ...
    async notifyLocalViewChanges(viewChanges) {
        try {
            ...
            if (isIndexedDbTransactionError(e)) {
                ...
                logDebug(LOG_TAG$5, 'Failed to update sequence numbers: ' + e);
            }
            else {
                throw e;
            }
    ...
    async releaseTarget(targetId, keepPersistedTargetData) {
        ...
        try {
            ...
            if (isIndexedDbTransactionError(e)) {
                ...
                logDebug(LOG_TAG$5, `Failed to update sequence numbers for target ${targetId}: ${e}`);
            }
            else {
                throw e;
            }
...
function getNewDocumentChanges(localStore) {
    const localStoreImpl = debugCast(localStore);
    const remoteDocumentCacheImpl = debugCast(localStoreImpl.remoteDocuments);
    return localStoreImpl.persistence
        .runTransaction('Get new document changes', 'readonly', txn => remoteDocumentCacheImpl.getNewDocumentChanges(txn, localStoreImpl.lastDocumentChangeReadTime))
 ...
 */
// PORTING NOTE: Multi-Tab only.
async function synchronizeLastDocumentChangeReadTime(localStore) {
    const localStoreImpl = debugCast(localStore);
    const remoteDocumentCacheImpl = debugCast(localStoreImpl.remoteDocuments);
    return localStoreImpl.persistence
        .runTransaction('Synchronize last document change read time', 'readonly', txn => remoteDocumentCacheImpl.getLastReadTime(txn))
        .then(readTime => {
        localStoreImpl.lastDocumentChangeReadTime = readTime;
    });
}
 ...
 * limitations under the License.
 */
const LOG_TAG$6 = 'ExponentialBackoff';
/**
...
class AsyncQueue {
    ...
    async retryNextOp() {
        ...
        try {
            ...
            if (isIndexedDbTransactionError(e)) {
                logDebug(LOG_TAG$7, 'Operation failed with retryable error: ' + e);
            }
            else {
...
class SyncEngineImpl {
    ...
    async registerPendingWritesCallback(callback) {
        if (!this.remoteStore.canUseNetwork()) {
            logDebug(LOG_TAG$8, 'The network is disabled. The task returned by ' +
    ...
    updateTrackedLimbos(targetId, limboChanges) {
        for (const limboChange of limboChanges) {
            if (limboChange instanceof AddedLimboDocument) {
                ...
                logDebug(LOG_TAG$8, 'Document no longer in limbo: ' + limboChange.key);
    ...
    trackLimboChange(limboChange) {
        ...
        if (!this.activeLimboTargetsByKey.get(key)) {
            logDebug(LOG_TAG$8, 'New document in limbo: ' + key);
    ...
    async handleCredentialChange(user) {
        ...
        if (userChanged) {
            logDebug(LOG_TAG$8, 'User change. New user:', user.toKey());
...
async function synchronizeViewAndComputeSnapshot(syncEngine, queryView) {
    ...
    const queryResult = await syncEngineImpl.localStore.executeQuery(queryView.query, 
    /* usePreviousResults= */ true);
    const viewSnapshot = queryView.view.synchronizeWithPersistedState(queryResult);
    if (syncEngineImpl.isPrimaryClient) {
        syncEngineImpl.updateTrackedLimbos(queryView.targetId, viewSnapshot.limboChanges);
    }
    return viewSnapshot;
}
/** Applies a mutation state to an existing batch.  */
// PORTING NOTE: Multi-Tab only.
async function applyBatchState(syncEngine, batchId, batchState, error) {
    const syncEngineImpl = debugCast(syncEngine);
    syncEngineImpl.assertSubscribed('applyBatchState()');
    const documents = await lookupMutationDocuments(syncEngineImpl.localStore, batchId);
    if (documents === null) {
        ...
        // as applicable.
        logDebug(LOG_TAG$8, 'Cannot apply mutation batch with id: ' + batchId);
        return;
    }
...
// PORTING NOTE: Multi-Tab only.
async function applyPrimaryState(syncEngine, isPrimary) {
    const syncEngineImpl = debugCast(syncEngine);
        ...
        syncEngineImpl.queriesByTarget.forEach((_, targetId) => {
            if (syncEngineImpl.sharedClientState.isLocalQueryTarget(targetId)) {
                ...
                p = p.then(() => {
                    ...
                    return syncEngineImpl.localStore.releaseTarget(targetId, 
                    /*keepPersistedTargetData=*/ true);
                });
            }
...
async function synchronizeQueryViewsAndRaiseSnapshots(syncEngine, targets, transitionToPrimary) {
    ...
    for (const targetId of targets) {
        ...
        if (queries && queries.length !== 0) {
            ...
            targetData = await syncEngineImpl.localStore.allocateTarget(queryToTarget(queries[0]));
            for (const query of queries) {
            ...
            await syncEngineImpl.initializeViewAndComputeSnapshot(synthesizeTargetToQuery(target), targetId, 
            /*current=*/ false);
        }
...
/** Applies a query target change from a different tab. */
// PORTING NOTE: Multi-Tab only.
async function applyTargetState(syncEngine, targetId, state, error) {
    const syncEngineImpl = debugCast(syncEngine);
    if (syncEngineImpl._isPrimaryClient) {
        // If we receive a target state notification via WebStorage, we are
        // either already secondary or another tab has taken the primary lease.
        logDebug(LOG_TAG$8, 'Ignoring unexpected query state notification.');
        return;
    }
    if (syncEngineImpl.queriesByTarget.has(targetId)) {
        switch (state) {
            ...
            case 'not-current': {
                ...
                await syncEngineImpl.emitNewSnapsAndNotifyLocalStore(changes, synthesizedRemoteEvent);
                break;
            }
            case 'rejected': {
                await syncEngineImpl.localStore.releaseTarget(targetId, 
                /* keepPersistedTargetData */ true);
...
async function applyActiveTargetsChange(syncEngine, added, removed) {
    const syncEngineImpl = debugCast(syncEngine);
    if (!syncEngineImpl._isPrimaryClient) {
        return;
    }
    for (const targetId of added) {
        if (syncEngineImpl.queriesByTarget.has(targetId)) {
            ...
            logDebug(LOG_TAG$8, 'Adding an already active target ' + targetId);
            continue;
        }
        const target = await getCachedTarget(syncEngineImpl.localStore, targetId);
        const targetData = await syncEngineImpl.localStore.allocateTarget(target);
        await syncEngineImpl.initializeViewAndComputeSnapshot(synthesizeTargetToQuery(target), targetData.targetId, 
        /*current=*/ false);
        syncEngineImpl.remoteStore.listen(targetData);
    }
...
class DatastoreImpl extends Datastore {
    constructor(credentials, serializer) {
        super();
...
function newDatastore(credentials, serializer) {
...
class MultiTabOfflineComponentProvider extends IndexedDbOfflineComponentProvider {
    constructor(onlineComponentProvider) {
        super();
    ...
    async initialize(cfg) {
        ...
        await this.persistence.setPrimaryStateListener(async (isPrimary) => {
            await applyPrimaryState(this.onlineComponentProvider.syncEngine, isPrimary);
            if (this.gcScheduler) {
...
function newQueryFilter(query, methodName, dataReader, databaseId, fieldPath, op, value) {
    ...
    if (fieldPath.isKeyField()) {
        ...
        fieldValue = parseQueryValue(dataReader, methodName, value, op === "in" /* IN */);
    }
...
var version$1 = "1.16.3-20207421553";
...
function registerFirestore(instance) {
    configureForFirebase(instance, (app, auth) => {
        ...
        const offlineComponentProvider = new MultiTabOfflineComponentProvider(onlineComponentProvider);
